...
// Sidebar menu items for different roles
...
const sidebarMenus: SidebarMenus = {
  admin: [
    ...
    },
    {
      ...
      icon: (
        <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="8" cy="8" r="4"/><circle cx="16" cy="8" r="4"/><path d="M2 20c0-2.5 3-4.5 6-4.5s6 2 6 4.5"/><path d="M14 20c0-2.5 3-4.5 6-4.5s6 2 6 4.5"/></svg>
      ),
      href: '/admin/staff',
    },
  ],
...
export default function DashboardLayout({
  ...
  const menus: SidebarMenuItem[] = sidebarMenus[role] || [];
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 font-sans">
      {/* Sidebar */}
      <aside
        className={`flex flex-col justify-between bg-white border-r border-gray-200 transition-all duration-300 ease-in-out
          ${collapsed ? 'w-16' : 'w-48'}
          fixed left-0 top-0 bottom-0 z-30 shadow-sm
        `}
      >
        ...
        {/* Toggle button */}
        <button
          className={`mb-4 mx-auto flex items-center justify-center w-8 h-8 rounded-full transition-colors hover:bg-orange-100`}
          onClick={() => setCollapsed((c) => !c)}
          aria-label="Toggle sidebar"
        >
          {collapsed ? (
            // Right arrow
            <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 6l6 6-6 6" className="text-black"/></svg>
          ) : (
            // Left arrow
            <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M15 6l-6 6 6 6" className="text-black"/></svg>
          )}
        </button>
      </aside>
      ...
      <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ease-in-out ${collapsed ? 'ml-16' : 'ml-48'}`}>
        ...
        <header className="flex items-center justify-between h-16 px-8 bg-gray-900 text-white shadow-sm">
          <div className="flex items-center gap-4">
            {/* TEXTS logo: T (white), EX (orange), TS (white) */}
...
// To use Poppins or Inter instead of Geist, swap the font import and update the font-sans class in Tailwind config or globals.css. 