...
export default function LogisticsAnalyticsDashboard() {
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      // Fetch order status logs for timing analysis
      ...
      // Calculate key metrics with status logs
      calculateKeyMetrics(orders || [], statusLogs || [])
      calculateStatusDistribution(orders || [])
      calculateVehicleUtilization(orders || [])
      calculateTopRoutes(orders || [])
      calculateDriverPerformance(orders || [], statusLogs || [])
      calculatePriorityBreakdown(orders || [])
  ...
  // Helper function to get unique status changes (filter duplicates)
  const getUniqueStatusChanges = (logs: any[], orderId: string) => {
    ...
    // Logs are already sorted DESC, so we reverse to get chronological order
  ...
  // Helper to calculate time between two statuses
  const getTimeBetweenStatuses = (logs: any[], orderId: string, fromStatus: string, toStatus: string) => {
    ...
    return Math.round((toTime - fromTime) / 60000) // minutes
  }

  const calculateKeyMetrics = (orders: any[], statusLogs: any[]) => {
    const totalOrders = orders.length
    const deliveredOrders = orders.filter(o => o.status === 'delivered')
    
    // Calculate actual delivery time using status logs
    let totalDeliveryTime = 0
    let validDeliveryTimeCount = 0

    deliveredOrders.forEach(order => {
      const timeToDeliver = getTimeBetweenStatuses(
        statusLogs, 
        order.id, 
        'order_placed', 
        'delivered'
    ...
    const avgTime = validDeliveryTimeCount > 0
      ? Math.round(totalDeliveryTime / validDeliveryTimeCount)
      : 0

    // Calculate true on-time rate using delivery windows
    let onTimeCount = 0
    ...
    const onTimeRate = measurableOrders > 0 
      ? (onTimeCount / measurableOrders) * 100 
      : 0

    // Count unique routes (pickup_area → dropoff combinations)
    const uniqueRoutes = new Set(
      orders.flatMap(o => 
        (o.dropoffs || []).map((d: any) => `${o.client?.pickup_area}-${d.dropoff_address}`)
      )
    )
  ...
  const calculateVehicleUtilization = (orders: any[]) => {
  ...
  const calculateTopRoutes = (orders: any[]) => {
  ...
  const calculateDriverPerformance = (orders: any[], statusLogs: any[]) => {
        ...
        if (order.status === 'delivered') {
          ...
          if (order.delivery_window_end) {
            ...
            if (deliveredLog) {
              ...
              if (deliveredTime <= windowEnd) {
                driverStats[driverId].onTimeDeliveries += 1
              }
            }
          }
          ...
          if (deliveryTime) {
            driverStats[driverId].totalDeliveryTime += deliveryTime
          }
        }
      }
    })
    ...
    const performance = Object.values(driverStats)
      .map(driver => ({
        ...
        onTime: driver.delivered > 0 
          ? Math.round((driver.onTimeDeliveries / driver.delivered) * 100) 
          : 0,
        avgTime: driver.delivered > 0
          ? Math.round(driver.totalDeliveryTime / driver.delivered)
          : 0
      }))
  ...
  const calculatePriorityBreakdown = (orders: any[]) => {
      ...
      >
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">Logistics Analytics</h1>
              <p className="text-gray-600">Delivery performance, route efficiency, and lead times</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
            ...
            <h3 className="text-sm font-medium text-gray-600 mb-1">Processing Rate</h3>
